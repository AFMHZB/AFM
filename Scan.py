...
class Scan:
    def __init__(self, scan_dict):
        ...
        self.OBSERVER_LABEL = ['live_afm', 'img_view', 'plot_view', 'progress', 'live_plot', 'add_step', 'finish_step']
        self.live_plot = True
        self.STEP_MAP = {}
        self.exit = False
        self.observers = {}
        ...
        self.hdf5_dict['Info']['Control'] = scan_dict['Control']
        self.hdf5_dict['Data'] = {}
    ...
    def add_step(self, text):
        for callback in self.observers['add_step']:
            callback(text)
            
    def finish_step(self, text):
        for callback in self.observers['finish_step']:
            callback(text)
    ...
    def get_step_image(self, step, channel='Z'):
        target = self.STEP_MAP[step]
        if target[0] == 'list':
            image = cv2.normalize(target[1][channel], None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
            image = cv2.resize(image, self.preview_size)
        elif target[0] == 'img':
            image = cv2.resize(target[1], self.preview_size)
        else:
            print('Error, da stimmt was nicht mit der STEP_MAP im Image')
        return image
    
    def get_plot_image(self, step, channel='O2A'):
        self.live_plot = False
        target = self.STEP_MAP[step]
        if target[0] == 'list':
            data = target[1][channel]
        else:
            print('Error, da stimmt was nicht mit der STEP_MAP im Plot')
        return data
    
    def set_live_plot(self, value):
        self.live_plot = value
    
    def scan_setup(self, step):
        #self.set_live_image(np.zeros(self.preview_size))
        now = datetime.now().strftime('%Y-%m-%d %H%M')
        self.hdf5_dict['Info']['Datetime'] = now
        os.makedirs(self.hdf5_dict['Info']['Control']['Dest_path'], exist_ok = True)
        all_shorts = []
        ...
        scan_name = '{} {}_{}_{}µm_{}px_{}-{}'.format(now, ops, self.hdf5_dict['Info']['Project'], round(float(scanarea)), round(float(pixelarea)), round(float(step)), round(float(self.hdf5_dict['Info']['Control']['Iterations'])))
        self.hdf5_path = os.path.join(self.hdf5_dict['Info']['Control']['Dest_path'], scan_name + '.hdf5')
    ...
    def afm_scan(self):
        ...
        if not afm_data == {}:
            data_path = os.path.join(self.hdf5_dict['Info']['Control']['Dest_path'], self.hdf5_dict['Info']['Datetime'] + '_CSV')
    ...
    def full_scan(self, step, csv_path=''):
        try:
            ...
            new_meas = (csv_path == '')
            if new_meas:
                step = 'Overview AFM Scan'
                self.add_step(step)
                self.afm_data = self.afm_scan()
                self.finish_step(step)
                self.STEP_MAP[step] = ('list', self.afm_data)
            else:
                self.afm_data = {}
                for root, dirs, files in os.walk(csv_path):
                    for f in files:
                        c = f.split('.')[0]
                        self.afm_data[c] = np.loadtxt(os.path.join(root, f), delimiter=',')
                
            if 'Z' in self.afm_data.keys() and 'R-Z' in self.afm_data.keys():
                ratio = self.hdf5_dict['Info']['AFM']['Dx'] / self.hdf5_dict['Info']['AFM']['Px'] #um / px
                
                step = 'Searching for Bacteria'
                self.add_step(step)
                f = FindBacteria(self.hdf5_dict['Info']['Characteristics'], ratio)
                data = f.full_correction(self.afm_data['Z'], self.afm_data['R-Z'], self.hdf5_dict['Info']['AFM']['Hlimit'] * 10**(-6))
                bac_found = f.find_bacteria(data, self.hdf5_dict['Info']['AFM']['Hlimit'] * 10**(-6))
                self.hdf5_dict['Data'] = f.get_dict()
                self.finish_step(step)
                self.STEP_MAP[step] = ('img', self.hdf5_dict['Data']['Bacteria_IMG'])
                if bac_found:
                    for key in self.hdf5_dict['Data']['Bacteria'].keys():
                        self.neaConnect.set_meas_completed(False)
                        #get the absolute coords for the relative points
                        for k in self.hdf5_dict['Data']['Bacteria'][key]['Points'].keys():
                            newx = ((self.hdf5_dict['Data']['Bacteria'][key]['Points'][k]['Coord'][0] * ratio) - (self.hdf5_dict['Info']['AFM']['Dx'] / 2)) + self.hdf5_dict['Info']['AFM']['X0']
                            newy = ((self.hdf5_dict['Data']['Bacteria'][key]['Points'][k]['Coord'][1] * ratio) - (self.hdf5_dict['Info']['AFM']['Dy'] / 2)) + self.hdf5_dict['Info']['AFM']['Y0']
                            newx = round(newx, 2)
                            newy = round(newy, 2)
                            self.hdf5_dict['Data']['Bacteria'][key]['Points'][k]['Coord'] = (newx, newy)
                            
                        if new_meas:
                            (x0, y0) = self.hdf5_dict['Data']['Bacteria'][key]['Points']['Center']['Coord']
                            dxy = self.hdf5_dict['Data']['Bacteria'][key]['Dxy']
                            res = self.hdf5_dict['Data']['Bacteria'][key]['Pxy']
                            
                            ##hier step einfügen
                            #self.set_cur_image(self.hdf5_dict['Data']['Bacteria'][key]['Meassurement_Points_IMG'])
                            
                            ##hier neu starten
                            
                            while not self.neaConnect.get_meas_completed():
                                step = 'Control AFM Scan ' + key
                                self.add_step(step)
                                self.spec_data = self.neaConnect.scanAFM(x0, y0, dxy, dxy, res, res, 0, self.hdf5_dict['Info']['AFM']['T_int'], self.hdf5_dict['Info']['AFM']['Setpoint'],
                                                            self.hdf5_dict['Info']['AFM']['Hlimit'], channel_names = self.channel)
                                
                                self.hdf5_dict['Data']['Bacteria'][key]['AFM'] = {}
                                for k in self.spec_data.keys():
                                    self.spec_data[k] = asNumpyArray(self.spec_data[k])
                                    self.hdf5_dict['Data']['Bacteria'][key]['AFM'][k] = self.spec_data[k]
                                    
                                self.finish_step(step)
                                self.STEP_MAP[step] = ('list', self.spec_data)
                                
                                step = 'Validate Bacteria ' + key
                                self.add_step(step)
                                data = f.full_correction(self.spec_data['Z'], self.spec_data['R-Z'], self.hdf5_dict['Info']['AFM']['Hlimit'] * 10**(-6))
                                bac_still_there = f.find_bacteria(data, self.hdf5_dict['Info']['AFM']['Hlimit'] * 10**(-6))
                                self.small_bact_dict = f.get_dict()
                                self.finish_step(step)
                                self.STEP_MAP[step] = ('img', self.small_bact_dict['Bacteria_IMG'])
                                
                                if bac_still_there:
                                    for k in self.small_bact_dict['Bacteria']['Bacteria1']['Points'].keys():
                                        newx = ((self.small_bact_dict['Bacteria']['Bacteria1']['Points'][k]['Coord'][0] * ratio) - (dxy / 2)) + x0
                                        newy = ((self.small_bact_dict['Bacteria']['Bacteria1']['Points'][k]['Coord'][1] * ratio) - (dxy / 2)) + y0
                                        self.hdf5_dict['Data']['Bacteria'][key]['Points'][k]['Coord'] = (newx, newy)
                                    ...
                                    for k in self.hdf5_dict['Data']['Bacteria'][key]['Points'].keys():
                                        ...
                                        self.hdf5_dict['Data']['Bacteria'][key]['Points'][k]['Current'] = epics.caget('CUM1ZK3RP:rdCur')
                                        print(k)
                                        
                                        step = 'Interferogram ' + k + ' ' + key
                                        self.add_step(step)
                                        self.fourier_data = self.neaConnect.scan_fourier(self.hdf5_dict['Data']['Bacteria'][key]['Points'][k]['Coord'][0], self.hdf5_dict['Data']['Bacteria'][key]['Points'][k]['Coord'][1], 0, 0,
                                                                                **self.hdf5_dict['Info']['Fourier'], channel_names = self.channel)
                                        if self.neaConnect.get_wait_for_injection():
                                            self.add_step('Wait for Injection')
                                            print('Unterbrochen wegen Epics')
                                            break
                                        
                                        for fk in self.fourier_data.keys():
                                            self.fourier_data[fk] = asNumpyArray(self.fourier_data[fk])
                                            self.hdf5_dict['Data']['Bacteria'][key]['Points'][k][fk] = self.fourier_data[fk]
                                        
                                        self.finish_step(step)
                                        self.STEP_MAP[step] = ('list', self.hdf5_dict['Data']['Bacteria'][key]['Points'][k])
                                ...
                                while self.neaConnect.get_wait_for_injection():
                                    time.sleep(0.5)
                                self.finish_step('Wait for Injection')
                                        
                        else:
                            print('Komisches Else')
                            #self.set_cur_image(self.hdf5_dict['Data']['Bacteria'][key]['Meassurement_Points_IMG'])
                
                self.hdf5_dict['Data'] = self.hdf5_dict['Data']
            
            else:
                print('Z and/or R-Z is not in Channel, therefore can not proceed.')

            self.hdf5_dict['Data']['AFM'] = self.afm_data
    ...
    def test_whitelight(self):
        self.neaConnect = NeaSNOMConnect('192.168.89.44', os.path.join(os.getcwd(), 'updates/SDK/'), con_needed = True)
        self.neaConnect.bind_to('Progress', self.set_progress)
        self.neaConnect.bind_to('Cur_Data', self.set_live_image)
        afm_data = self.neaConnect.scan_whitelight(**self.hdf5_dict['Info']['AFM'], channel_names = self.channel)
